...
def experiment(batch_list=[16,32,64,128], epochs=5):
    ...
    for B in batch_list:
        ...
        t0 = time.time()
        history = train(model, X_train, y_train, X_val, y_val, epochs=epochs, batch=B, lr=1e-3, verbose=False)
        t = time.time() - t0
        results[B] = (t, history['val_acc'][-1])
        print(f"B={B} time={t:.2f}s val_acc={history['val_acc'][-1]:.4f}")
    return results
